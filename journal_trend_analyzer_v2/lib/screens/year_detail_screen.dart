...
class _YearDetailScreenState extends State<YearDetailScreen> {
  ...
  String _topJournalShortName(AppStrings s) {
    if (_papers.isEmpty) return s.na;
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text(s.yearLabel(widget.year)),
      ),
      body: _loading
          ? AppLoadingView(
              fillScreen: false,
              expand: true,
              message: s.loadingYearData,
            )
          : ListView(
              ...
              children: [
                ...
                if (_error != null) ...[
                  ...
                  TextButton(onPressed: _loadInitial, child: Text(s.retry)),
                ],
                  ...
                  Wrap(
                    ...
                    children: _hotTopics
                        .map(
                          (topic) => ActionChip(
                            label: Text(
                              '${topic.name} (${formatOpenAlexCount(topic.count)} ${s.metricPublications})',
                            ),
                ...
                const SizedBox(height: 20),
                Text(
                  s.topCitedPapersInYear(widget.year),
