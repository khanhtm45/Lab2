...
class OpenAlexService {
  OpenAlexService(
    OpenAlexConfig config, {
    AppPreferences? preferences,
  })  : _config = config,
        _preferences = preferences;

  OpenAlexService.legacy([OpenAlexConfig? config])
  ...
  int get _listPageSize => _preferences?.resultsPerPage ?? listPageSize;
  ...
  List<SankeyFlow> _buildJournalMigrationFlows(List<Map<String, dynamic>> works) {
    ...
    final yearJournals = <int, Map<String, int>>{};
    ...
    for (final work in works) {
      ...
      yearJournals.putIfAbsent(year, () => {});
      yearJournals[year]![journal] = (yearJournals[year]![journal] ?? 0) + 1;
    }

    final years = yearJournals.keys.toList()..sort();
    final flows = <SankeyFlow>[];

    String? topJournalForYear(int year) {
      final counts = yearJournals[year];
    ...
    for (var i = 0; i < years.length; i++) {
      ...
      for (final entry in sorted.take(3)) {
        flows.add(
          SankeyFlow(
            source: '$year',
            target: _shortLabel(entry.key, 20),
            value: entry.value.toDouble(),
          ),
        );
      }
      if (i > 0) {
        final prevTop = topJournalForYear(years[i - 1]);
        final currTop = topJournalForYear(year);
        if (prevTop != null && currTop != null) {
          flows.add(
            SankeyFlow(
              source: _shortLabel(prevTop, 20),
              target: _shortLabel(currTop, 20),
              value: (counts[currTop] ?? 1).toDouble(),
            ),
          );
        }
      }
  ...
  List<String> _topicsFromWork(Map<String, dynamic> work) {
    ...
    for (final item in work['concepts'] as List? ?? []) {
      ...
      if (name != null && name.isNotEmpty && score >= 0.25) {
        topics.add(name);
      }
    }
    if (topics.isEmpty) {
  ...
  Future<TopicComparisonResult> compareTopics(
    ...
    final results = await Future.wait([
      ...
      fetchWorksGroupedCounts(groupBy: groupByCountry, search: topicA, limit: 1),
      fetchWorksGroupedCounts(groupBy: groupByCountry, search: topicB, limit: 1),
      fetchPublicationTrendByYear(search: topicA),
      fetchPublicationTrendByYear(search: topicB),
    ]);

    final journalsA = results[6] as List;
    final journalsB = results[7] as List;
    final countriesA = results[8] as List;
    final countriesB = results[9] as List;
    final trendA = results[10] as Map<int, int>;
    final trendB = results[11] as Map<int, int>;

    int peakYear(Map<int, int> trend) {
    ...
    return TopicComparisonResult(
      ...
      journalsA: journalsA.length,
      journalsB: journalsB.length,
      topCountryA: countriesA.isEmpty ? '' : countriesA.first.name,
      topCountryB: countriesB.isEmpty ? '' : countriesB.first.name,
      topJournalNameA: journalsA.isEmpty ? '' : journalsA.first.name,
      topJournalNameB: journalsB.isEmpty ? '' : journalsB.first.name,
      peakYearA: peakYear(trendA),
      peakYearB: peakYear(trendB),
    );
  }
  ...
  /// Lightweight health check against the OpenAlex works endpoint.
  Future<bool> testConnection() async {
    try {
      await fetchWorksTotalCount();
      return true;
    } catch (_) {
      return false;
    }
  ...
  Future<List<OpenAlexRankedEntity>> fetchJournalsForYear({
    required int year,
    String? search,
    bool globalInfluential = false,
    int limit = 5,
  ...
  Future<JournalSourceProfile?> fetchSourceProfile(String sourceId) async {
    final id = shortOpenAlexId(sourceId);
    if (id.isEmpty) return null;

    final data = await _getJson(
      _openAlexUri('/sources/$id', _authParams()),
    );

    final issnList = data['issn'] as List?;
    final issn = data['issn_l']?.toString() ??
        (issnList != null && issnList.isNotEmpty
            ? issnList.first.toString()
            : '');

    final rawType = data['type']?.toString() ?? 'journal';
    final sourceType = rawType
        .split('-')
        .map(
    ...
    return JournalSourceProfile(
      name: data['display_name']?.toString() ?? 'Unknown Journal',
      publisher: data['host_organization_name']?.toString() ??
          data['publisher']?.toString() ??
          'N/A',
      sourceType: sourceType,
      issn: issn.isEmpty ? 'N/A' : issn,
    );
  }

  Future<double?> fetchSourceOpenAccessPercent({
    required String sourceId,
    String? search,
  }) async {
    final data = await _fetchWorksGroupBy(
      groupBy: groupByOpenAccess,
      search: search,
      globalInfluential: false,
    ...
    final counts = parseGroupByKeyCounts(data, limit: 4);
    var open = 0;
    var closed = 0;
  ...
  Future<double> fetchSourceAverageCitation({
    required String sourceId,
    String? search,
  }) async {
    final page = await _fetchWorksPage(
  ...
  Future<AuthorProfile?> fetchAuthorDetailProfile(String authorId) async {
    final id = shortOpenAlexId(authorId);
    if (id.isEmpty) return null;
    ...
    final name = data['display_name']?.toString() ?? 'Unknown Author';

    final institutions = data['last_known_institutions'] as List? ?? [];
    var institution = 'N/A';
    if (institutions.isNotEmpty && institutions.first is Map) {
      institution = (institutions.first as Map)['display_name']
              ?.toString()
              .trim() ??
          'N/A';
    }
    if (institution.isEmpty) institution = 'N/A';

    final ids = data['ids'] as Map?;
    final orcidRaw = ids?['orcid']?.toString();
    String? orcid;
    if (orcidRaw != null && orcidRaw.isNotEmpty) {
      orcid = orcidRaw.replaceFirst('https://orcid.org/', '');
    }

    final concepts = data['x_concepts'] as List? ?? [];
    final conceptEntries = <({String name, double score})>[];
    for (final concept in concepts) {
      if (concept is! Map) continue;
      final displayName = concept['display_name']?.toString();
      final score = (concept['score'] as num?)?.toDouble() ?? 0;
      if (displayName != null && displayName.isNotEmpty) {
        conceptEntries.add((name: displayName, score: score));
      }
    }
    conceptEntries.sort((a, b) => b.score.compareTo(a.score));
    final researchAreas =
        conceptEntries.take(5).map((entry) => entry.name).toList();

    final publicationCount = (data['works_count'] as num?)?.toInt() ?? 0;
    final citationCount = (data['cited_by_count'] as num?)?.toInt() ?? 0;
    final averageCitations =
        publicationCount > 0 ? citationCount / publicationCount : 0.0;
  ...
  Future<RankedAuthorEntry> enrichAuthorRank(OpenAlexRankedEntity author) async {
    final id = shortOpenAlexId(author.id);
    if (id.isEmpty) {
      return RankedAuthorEntry(
        entity: author,
    ...
    try {
      ...
      return RankedAuthorEntry(
        entity: author,
        publicationCount: author.count,
        citationCount: (data['cited_by_count'] as num?)?.toInt() ?? 0,
        institution: institution,
      );
    } catch (_) {
      return RankedAuthorEntry(
  ...
  Future<List<RankedAuthorEntry>> enrichAuthorRanks(
    List<OpenAlexRankedEntity> authors,
  ) async {
    if (authors.isEmpty) return [];
    return Future.wait(authors.map(enrichAuthorRank));
  }
  ...
  /// Yearly publication counts for top emerging topics — multi-series line charts.
  Future<Map<String, Map<int, int>>> fetchEmergingTopicTrendSeries({
    required List<OpenAlexRankedEntity> concepts,
    String? search,
    bool globalInfluential = false,
    int limit = 4,
  }) async {
    final series = <String, Map<int, int>>{};
    for (final concept in concepts.take(limit + 4)) {
  ...
  Map<String, String> _searchParams({
    ...
    if (filters?.publicationTypes.isNotEmpty ?? false) {
      filterParts.add('type:${filters!.publicationTypes.join('|')}');
    } else if (filters?.publicationType != null &&
  ...
  Future<OpenAlexWorksResult> _fetchWorksResultPage(
    ...
    final resolvedPerPage = perPage ?? _listPageSize;
